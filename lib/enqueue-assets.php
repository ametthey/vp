<?php
	...
	***********************************************************************/
	...
	function test_assets() {
		...
		wp_enqueue_script( 'test-scripts', get_template_directory_uri() . '/dist/assets/js/main.js', array('jquery'), '1.0.1' ,   true );
	...
	***********************************************************************/
    ...
    function test_register_assets() {
        wp_enqueue_style( 'test-register-stylesheet', get_template_directory_uri() . '/dist/assets/css/register.css', array(), '1.0.0' , 'all' );
    }
    add_action('login_enqueue_scripts', 'test_register_assets');
	...
	***********************************************************************/
	...
	function test_remove_block_library_css(){
		wp_dequeue_style( 'wp-block-library' );
	}
	add_action( 'wp_enqueue_scripts', 'test_remove_block_library_css' );
	...
	***********************************************************************/
	...
	function test_defer_parsing_of_js( $url ) {
		if ( is_user_logged_in() ) return $url; //don't break WP Admin
		if ( FALSE === strpos( $url, '.js' ) ) return $url;
		if ( strpos( $url, 'jquery.js' ) ) return $url;
		return str_replace( ' src', ' defer src', $url );
	}
	add_filter( 'script_loader_tag', 'test_defer_parsing_of_js', 10 );
